...
def dqap1_raw(data_recent):
    """
    This measurement function builds the DQAP1 list.
    """
...
def dqap2_raw(data_recent):
    """
    This measurement function builds the DQAP2 list.
    """
    meas_obj = data_recent
    if not dqap2_init:
        return val_tag()
...
def paros_raw(meas_obj):
    """
    This measurement function builds the paros list.
    """
    if not paros_init:
        return val_tag()
...
def vis_raw(meas_obj):
    """
    This measurement function builds the visibility list.
    """
    if not vis_init:
        return val_tag()
